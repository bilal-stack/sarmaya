...
- Residual risk: rows written before this change remain in the server's local zone, so timestamps that straddle the change are not directly comparable. For the non-hashed tables the correction is
  `UPDATE <table> SET created_at = created_at AT TIME ZONE '<original server zone>' AT TIME ZONE 'UTC';`
  which is deliberately **not** run here. `audit_logs.timestamp` must not be corrected this way while chain verification is expected to pass.
- Owner: bilal (dev)
